...
async def get_all_TicketType(current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            cur.execute("""
...
async def get_TicketType_by_id(tickettype_id: int, current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            cur.execute("""
...
async def get_TicketType_by_EventID(event_id: int, current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            cur.execute("""
...
async def get_TicketType_by_Anything(value: str, current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            search = f"%{value}%"
