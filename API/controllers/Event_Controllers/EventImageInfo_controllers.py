...
async def get_all_eventimages(current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            cur.execute("""
...
async def get_eventimage_by_id(image_id: int, current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            cur.execute("""
