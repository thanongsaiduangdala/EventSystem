...
async def create_category(req_data: AddCategoryInfoRequest, current=Depends(require_permission("manage_categories"))):
    try:
        con = getConnect()
...
async def get_all_Categories(current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
...
async def get_category_by_id(category_id: int, current=Depends(require_permission("view_events"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
...
async def update_category(req_data: UpdateCategoryInfoRequest, current=Depends(require_permission("manage_categories"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
            sql = """
...
async def delete_category(category_id: int, current=Depends(require_permission("manage_categories"))):
    try:
        con = getConnect()
        with con.cursor() as cur:
